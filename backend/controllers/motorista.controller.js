...
exports.getRelatorioMotoristas = async (req, res) => {
    try {
        ...
        // Buscar viagens concluídas no período
        ...
        viagens.forEach(v => {
            ...
            // Calcular horas se a viagem está concluída
            const inicio = new Date(v.inicio);
            const fim = v.fim ? new Date(v.fim) : inicio;
            const horas = (fim.getTime() - inicio.getTime()) / (1000 * 60 * 60);
            totaisPorMotorista[motoristaId].horas += horas;
            totalHoras += horas;

            // Adicionar quilômetros
            const km = v.quilometrosPercorridos || 0;
            totaisPorMotorista[motoristaId].km += km;
            totalKm += km;
        });

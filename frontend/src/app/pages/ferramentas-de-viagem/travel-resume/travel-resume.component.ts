...
export class TravelResumeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.travelService.getViagem(id).subscribe(
      (data) => {
        ...
        const turnoId = this.viagem.turno._id;
  ...
  onSubmitFimViagem() {
    ...
    const id = this.viagem?._id;
    if (!id) return;
    this.travelService.updateFimViagem(id, dadosFim.horaFim, dadosFim.moradaFim).subscribe({
      next: (viagemAtualizada) => {
        this.viagem = viagemAtualizada;
        this.mostrarFormularioFim = false;
        // Optionally show a success message
        alert('Fim da viagem registado com sucesso!');
      },
      error: (err) => {
        alert('Erro ao registar fim da viagem: ' + (err.error?.message || err.message));
      }
    });
  }
}
